...
def xor(bits1, bits2):
    if len(bits1) != len(bits2):
        raise ValueError(f"Length mismatch: bits1 is {len(bits1)} bits, bits2 is {len(bits2)} bits")
    return [b1 ^ b2 for b1, b2 in zip(bits1, bits2)]

def shift_left(key, num_shifts):
    return key[num_shifts:] + key[:num_shifts]

def get_sbox_value(sbox_index, row, col):
    return S_BOXES[sbox_index][row][col]

def generate_round_keys(key):
    key_bits = list(map(int, bin(key)[2:].zfill(64)))
...
def s_box_substitution(xor_result):
    ...
    for i in range(8):
        ...
        s_box_value = get_sbox_value(i, row, col)

        substituted_bits.extend([int(bit) for bit in bin(s_box_value)[2:].zfill(4)])
